...
# Building #
rf_building <- ranger(BUILDINGID~.,
                       data = trainmodel[,WAPS_BUILD2],
                       mtry = 22)
Pred1_building <- predict(rf_building, data = validationData_1)
...
class_knn_latitude3 <- FNN::knn.reg(train = d_trainmodel[,WAPS_BUILD], 
                                    y= trainmodel$LATITUDE,
                                    test= d_validationData_1)

validationData_1$LATITUDE <- class_knn_latitude3[["pred"]]
d_validationData_1$LATITUDE <- class_knn_latitude3[["pred"]]

WAPS_BUILD_LAT <- grep("WAP|BUILDING|LATITUDE", names(d_trainmodel), value = T)

# LONGITUDE #
class_knn_longitude3 <- FNN::knn.reg(train = d_trainmodel[,WAPS_BUILD_LAT], 
...
pred1_floor <- predict(rf_floor, data = validationData_1)
